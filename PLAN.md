...
### v1.2（予定）：Xwear対応 — ボーン崩れ修正

**概要**

Xwear製アバター（非VRoid Studio標準出力のVRM）を変換した際に、メッシュへの激しいボーン崩れが発生する。

**症状**

- SLへアップロード後、頂点やメッシュが極端に引き伸ばされる・変形する
- 特定ボーン周辺の頂点が原点（0,0,0）へ収束するように崩壊する
- 通常のVRoid Studio出力では発生しない

**原因（推定）**

- XwearはVRoid Studioとは異なるボーン構造・命名規則を持つ場合がある
- ボーン名マッピングが不一致となり、ウェイト転送先がズレる可能性がある
- `inverseBindMatrices` の再計算にXwear固有の初期姿勢（Aポーズ以外）が影響している可能性がある
- 追加ボーン（Xwear独自拡張）のウェイトが削除や親への統合時に誤処理される可能性がある

**対応方針**

1. Xwear VRMの骨格構造・ボーン命名を調査し、VRoid 標準との差分を抽出する
2. 不明なボーンへのウェイトが正しく親ボーンへ再分配されているか検証する
3. `inverseBindMatrices` 再計算時の初期姿勢推定ロジックをXwear構造に対応させる
4. 必要であれば変換前の前処理ステップとしてXwear正規化パスを実装する
5. テストケースとして実際のXwear VRMファイルを使った自動検証を追加する

## 🧩 入出力仕様

**入力：** VRM 1.0（.vrm ファイル）

**出力：** glTF 2.0形式（.glbファイル、SLアップロード可能。daeは非対応）
...
- [x] 変換後GLBのテクスチャ同梱を確認（images/textures 参照が保持されることを確認）
- [x] 変換後アバター形状の破綻がないことを確認
...
- [x] ウェイト再分配・joints最適化の本実装
- [x] ボーン階層の再構成（SL向け最終階層を明示的に構築）
- [x] Bento拡張ボーン（目・口・指）の実運用レベル対応（まぶたは入力依存でベストエフォート）
- [x] 任意の3Dモデリングツールで再読込～SLアップロードの最終検証フロー完了（`--validation-checklist` で手順書生成対応）
...
- **形式：** glTF 2.0 JSON
- **パッケージ：** Second Lifeアップロード可能（.glbファイル）
...
1. **任意の3Dモデリングツールでの焼き込み（推奨）**
   - 変換後のglTFを任意の3Dモデリングツールで開く
